...
main(int argc, char* argv[])
{
  ...
  int blank_timeout;
	...
	for (i=0;i<tlen; i++)
                if (!isdigit(argv[5][i])) {
                        printf ("Entered blank_timeout value is not a number\n");
                        exit(1);
                }
	blank_timeout = atoi(argv[5]);
	printf("blank_timeout = %d seconds (after dimmer_timeout reached)\n", blank_timeout);
	 ...
	 * 4 - blank_timeout reached (screen has been blanked)
	...
	printf("Initialize brightness: %d\n", current_brightness);
	fprintf(brightfd, "%d\n", current_brightness);
	fflush(brightfd);
